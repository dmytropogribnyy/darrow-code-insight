...
This directory contains a deliberately small set of implementation excerpts selected from the current product codebase.
...
[`stale-chunk-recovery.ts`](stale-chunk-recovery.ts) is a production-facing resilience mechanism used by the web application.
...
- stores a short cooldown in `sessionStorage` to prevent reload loops;
