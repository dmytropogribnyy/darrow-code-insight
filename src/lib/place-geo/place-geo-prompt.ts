...
export function buildPlaceGeoPrompt(
  ...
  return [
    ...
    `6c. WORLD COVERAGE: the packet contains BOTH reachable-region and GLOBAL passes. When the`,
    `   strong matches sit on other continents (Americas, Asia, Africa, Oceania), say so plainly`,
    `   — never collapse the map to Europe out of habit. When a packet city is an ISLAND or an`,
    `   ocean location (Atlantic, Pacific, Indian, Mediterranean), name the island and its ocean/`,
    `   sea ("Funchal, on Madeira in the Atlantic") — island ground is a selling texture, use it`,
    `   when the data offers it.`,
    `6d. RECOGNIZABLE PLACES: lead with cities the reader can picture (capitals, major cities,`,
    `   known destinations — use the packet's population field as a guide). When the packet`,
    `   surfaces an obscure town or a city DISTRICT (e.g. "Kreuzberg", "Neue Neustadt"), do NOT`,
    `   present it as a standalone recommendation — fold it into its metro or nearest well-known`,
    `   packet city ("the Venus band around Berlin and Leipzig"). An unrecognizable name with no`,
    `   anchor is a failed recommendation.`,
