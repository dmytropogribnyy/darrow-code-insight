...
function coreV3Instructions(): string {
  return [
    ...
    "",
    "PER-SECTION PROOF TAGS (CORE-HUMAN-VOICE v3.2 — REQUIRED on every OBJECT section):",
    "- Add proof_tags: [string] with 2–5 short anchors to EVERY object section above.",
    "- proof_tags is the ONLY place technical machinery is allowed. The PDF renders it",
    "  as a compact grey evidence line at the tail of the section.",
    "- Each tag is a short human-readable phrase carrying the exact data, e.g.:",
    '  "Yang Earth Day Master", "Moon square Pluto (0.4° orb)", "Virgo Moon 2H",',
    '  "Xu day branch", "Aries Sun 9H (exalted)", "Life Path 3", "Expression 8".',
    "- Every proof tag must be REAL data from the chart. Never invent an anchor.",
    "- Exact orbs, degrees, element %, house numbers, Chinese branch/stem/life-stage",
    "  names, dignity/retrograde labels, exact aspect names → live HERE, never in prose.",
    "",
    ...
    "",
    "HUMAN READABILITY (HARD — body prose vs proof_tags is the whole point):",
    ...
    "- BODY PROSE MUST NOT CONTAIN: exact orbs ('0.4°', '0.27 degrees'), the word",
    "  'orb', element percentages ('54%'), raw house mechanics ('9th house stellium'),",
    "  Chinese branch/stem/life-stage detail ('Xu (Dog) branch', 'Ji-Chou', 'Grave",
    "  life stage'), nayin, or ten-gods labels. ALL of these go in proof_tags only.",
    "- META-METHOD language is BANNED in prose: no 'Five independent systems…',",
    "  'Two independent frameworks both arrive…', 'The numerology layer adds…',",
    "  'The BaZi layer confirms…'. SHOW the synthesis; never narrate the method.",
    "  (Method is stated ONCE on the Orientation page — not repeated per section.)",
    "- Machine words (architecture, system, layer, mechanism, configuration,",
    "  operating requirement, load-bearing) — at most ~2 per section. Prefer human",
    "  images (mountain, reservoir, terrain, current, weather).",
    "- A LIGHT mechanism mention in prose is fine ('the Moon-Pluto compression')",
    "  but WITHOUT numbers — the numbers belong in proof_tags.",
