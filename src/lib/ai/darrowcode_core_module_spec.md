...
This document defines the canonical structure, word targets, and content
requirements for the CORE module (Cosmic Core Code / UNVEIL).
...
Example: "Your essential factory settings: how you process pressure,
make decisions, and manage energy. The baseline for all further work."
Format: 1–2 sentences max. No data anchoring required.
...
- No mention of other products here

Tone: calm, confident, slightly warm. Like a trusted advisor opening
...
- At least 1 "Three independent systems point at the same truth" statement
- 1 PROTOCOL block
- Proof tags

Typical structure for a Cancer stellium + Water DM chart:
"In most people, there is an inner conflict: the mind wants one thing,
the heart another, actions a third. You are spared this friction. Your
nature is built on a rare [X] Lock: [list]. THE MEANING... THE
...
- 3 specific RECHARGE PROTOCOLS (concrete behaviors, not abstract advice)
- Warning Signal: what low battery looks like in observable behavior
- Proof tags

The recharge protocols must be actionable and specific:
Not "rest more" — but "Physical contact with water washes off psychic
static from others. It is your literal nervous system reset."
...
- The gap between how they appear (warm, approachable) and what runs behind
  the exterior (active scanning, security check)
- The cost of distributing empathy indiscriminately
- 1 PROTOCOL block
- Proof tags
...
- Life Path number + interpretation (long-arc path, the tension or harmony with other numbers)
- The central paradox or tension between the numbers (e.g., 8 outer + 7 inner)
- 1 PROTOCOL block
- Note: if full_name was not provided, calculate Life Path + Personal Year only,
  and add one sentence: "Provide your full name to unlock Expression, Soul Urge,
...
- The "feel first, verify later" sequence for this configuration
- What happens when the client tries to override this with pure logic
- 1 PROTOCOL block
- Proof tags
...
- Why the 12th house, fixed signs, or other Mars configurations create invisible
  or unconventional drive patterns
- The advantage of this tempo when used correctly
- 1 PROTOCOL block
- Proof tags

One grounded physical metaphor required. Example: "You are not a sports car
built for short sprints. You are a powerful diesel generator installed in a bunker."
...
- Mars-MC or other career tensions
- The "Gray Cardinal / Architect" frame if applicable
- The environment that activates vs. drains their professional function
- 1 PROTOCOL block
- Proof tags
...
- The psychological driver behind accumulation (security? influence? pleasure?)
- The primary trap in their money pattern
- 1 PROTOCOL block
- Proof tags
...
- The gap between what the client wants and how they appear to partners
- The "secret garden" principle (if applicable for 12th house Venus etc.)
- 1 PROTOCOL block
- Proof tags
...
- 2 specific environmental or behavioral requirements
- Warning Signal: how their system signals overload
- Proof tags

NOTE: Add this disclaimer in the section:
"This is interpretive orientation, not medical advice. Consult a healthcare
...
- 2 specific environment modes (e.g., "Water Sanctuary" + "Urban Incognito")
- 1 PROTOCOL block
- Proof tags
...
- The disarming move — what interrupts the loop
- "Warning Signal:" block (observable signs the loop has activated)
- Proof tags

This is NOT about moral failing. Frame as:
"Your primary zone of internal tension is the conflict between
[X] and [Y]. When [trigger], your system does not [obvious response].
It [actual response]. The loop..."
...
- No "you will achieve X"

Canonical form:
"Before: [specific inner experience without the map]
After: [same experience, now with structural context]"
...
- Decision Formula: The client's specific decision-making key
- The Core Conclusion: 1 sentence that crystallizes the whole architecture
- Brief timing note: current year/cycle and what it means for NOW
- Transition to the next product layer (soft, not sales)
...
- 1 specific named module or product that addresses the client's most
  ...
  pattern is flagged, reference MONEY module)
- No pricing, no urgency, no FOMO language
- End with: "Your next step → darrowcode.com"
...
## LOVEABLE PATCH INSTRUCTION

Paste this into Loveable:

```
Update the generate-report edge function for CORE module generation.

The current CORE structure uses 8 abstract sections with a total
word target of ~900 words. This is incorrect. Replace with the
following canonical structure.

CORE module must generate 17 sections (see CORE_MODULE_SPEC.md)
with a total target of 3,000–3,600 words.

The JSON output shape for CORE should be:

{
  "module": "CORE",
  "client_name": "...",
  "sections": {
    "cover_tagline": "string, 10–20 words",
    "orientation": "string, 180–220 words",
    "core_architecture": "string, 280–340 words",
    "battery": "string, 220–260 words",
    "social_interface": "string, 200–240 words",
    "numerology_code": "string, 260–300 words",
    "cognitive_style": "string, 200–240 words",
    "drive_and_rhythm": "string, 200–240 words",
    "professional_archetype": "string, 220–260 words",
    "money_and_value": "string, 200–240 words",
    "relationship_baseline": "string, 200–240 words",
    "vitality_baseline": "string, 180–220 words",
    "environment_and_resonance": "string, 180–220 words",
    "shadow_and_friction": "string, 240–280 words",
    "before_after": "string, 120–160 words",
    "executive_summary": "string, 280–320 words",
    "next_step": "string, 80–120 words"
  },
  "proof_tags": {
    "core_architecture": ["placement1", "placement2"],
    "battery": ["placement1"],
    ...
  }
}

Each section string = fully written prose with embedded proof tags
at the end, in this format:
[Sun conjunct Moon Cancer · Water Dominant 59% · Life Path 3]

Do not change: Stripe flow, PDF design, voice rules, API integration.
Only change: section structure and word targets in the AI user prompt
and the PDF template section mapping.
```
