...
async function callWithFallback(
  ...
  for (let attempt = 0; attempt < MAX_DEFAULT_ATTEMPTS; attempt++) {
    try {
      ...
      console.warn(
        `[anthropic] ${firstModel} attempt ${attempt + 1}/${MAX_DEFAULT_ATTEMPTS} failed (${e?.status ?? "net"}); retrying in ${wait}ms`,
      );
      await sleep(wait);
    }
  }
  ...
  if (fallbackModel && fallbackModel !== firstModel && isRetryable(lastErr)) {
    console.warn(
      `[anthropic] ${firstModel} exhausted, falling back to ${fallbackModel}:`,
      lastErr?.message,
    );
...
async function generateChunkedReport(
  ...
  const first = hasCore
    ? await (async () => {
        const split = await generateCoreV3Split(
          promptForModules(userPrompt, ["CORE"]),
          model,
          fallbackModel,
        );
  ...
  const restResults = await mapWithConcurrency(rest, 2, async (moduleCode) => {
    ...
    const result = await callWithFallback(
      promptForModules(userPrompt, [firstModule, moduleCode]),
      model,
      fallbackModel,
    );
  ...
  const merged: DarrowReport = {
    ...
    closing: {
      ...
      grand_synthesis:
        first.report.closing.grand_synthesis ?? first.report.closing.executive_summary,
    },
  };
  const parsed = DarrowReportSchema.safeParse(merged);
  if (!parsed.success)
    throw new Error(`Merged Anthropic chunks failed schema: ${parsed.error.message.slice(0, 400)}`);
  return {
    report: parsed.data,
    model_used: Array.from(
      new Set([first.model_used, ...restResults.map((r) => r.model_used)]),
    ).join("+"),
  };
}
