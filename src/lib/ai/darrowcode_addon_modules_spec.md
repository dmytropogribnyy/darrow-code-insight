...
Each add-on module renders **8–10 PDF pages · 1,200–1,500 words**.
(Current v2.1 target of 480–690 words was incorrect — less than half the needed volume.)

All add-on modules share the same 10-section skeleton, with content
specific to the module's domain.
...
- 12th house Venus or Neptune involvement if present
...
Proof tags emphasis: Venus, Mars, 7th house, 12th house, Venus-Neptune aspects
...
- Bazi Wealth element (Direct vs. Indirect Wealth structure)
...
Proof tags emphasis: 2nd house, 8th house, Jupiter, Saturn, Bazi Wealth element
...
"This module provides interpretive orientation for self-awareness and
lifestyle calibration only. It is not medical advice. Consult a qualified
...
- Saturn sign/house (discipline vs. depletion dynamic)
...
Proof tags emphasis: 6th house, Mars, Saturn, dominant element, Uranus/Neptune 6th
...
and the client's Personal Year number. If Personal Year is 5, this year
...
- Example: "Personal Year 5 creates the urge to change everything at once.
  ...
  without filtering it through the structural picture creates [X]."

Proof tags emphasis: Personal Year, active slow transits, Bazi luck cycle,
SR Ascendant
...
- Dominant element → material preferences
...
- The visual signature they already naturally gravitate toward
  (confirmed by chart, not invented)
...
IMPORTANT: Do NOT claim colors or stones have magical or healing properties.
Frame as: "For your system, [X] functions as a stabilizing signal because..."

Proof tags emphasis: Venus, Ascendant, Moon, dominant element, Bazi favorable element
...
- 9th house (foreign environments, expansion)
- Saturn line or IC line relevance
- Dominant element → landscape/climate preference (Water = coast/rivers,
...
NOTE: Do NOT recommend specific cities by name unless astrocartography
data with specific line coordinates was provided. Use environment types
and climate/landscape characteristics instead.

Proof tags emphasis: 4th house, 9th house, Venus line, IC line, dominant element

---

## JSON OUTPUT SHAPE (all add-ons)
...
```json
{
  ...
  "client_name": "...",
  "sections": {
    "module_opening": "string, 80–100 words",
    "primary_architecture": "string, 200–250 words",
    "mechanism": "string, 160–200 words",
    "key_pattern": "string, 160–200 words",
    "timing": "string, 120–150 words",
    "protocols": "string, 200–250 words",
    "shadow": "string, 140–180 words",
    "before_after": "string, 100–130 words",
    "summary": "string, 120–160 words",
    "bridge": "string, 60–80 words"
  },
  "proof_tags": {
    "primary_architecture": ["placement1", "placement2"],
    "shadow": ["aspect1"]
  }
}
```
...
```
Update the generate-report edge function for all add-on modules
(LOVE, MONEY, BODY, YEAR, STYLE, PLACE).

Current add-on word targets are 480–690 words. This is incorrect.
Replace with 1,200–1,500 words across 10 sections per add-on.

Use the shared 10-section skeleton from ADDON_MODULE_SPEC.md.
Apply module-specific content requirements from the same file.

The JSON shape for each add-on:
- 10 section keys (see spec)
- proof_tags object with per-section arrays

Each section = fully written prose. No bullet points except inside
PROTOCOL blocks.

Do not change: Stripe flow, PDF design, voice rules, API integration.
Only change: section structure and word targets for add-on modules.
```
