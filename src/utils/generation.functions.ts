// Generation pipeline.
// PROMPT 1C will replace `runPlaceholderGeneration` with the real
// astro-data → AI report → PDF → email pipeline. For now this produces
// a single-page placeholder PDF so the end-to-end flow is testable.
...
async function buildPlaceholderPdf(args: {
  ...
  page.drawText("Astro Report — Preview", {
    x: 50, y: 760, size: 22, font: serif, color: charcoal,
  });
  ...
  const lines: string[] = [
    ...
    `  • CORE`,
    ...args.modules.filter((m) => m !== ("CORE" as any)).map((m) => `  • ${m}`),
    "",
    ...
    "(astro data, narrative chapters, charts) is wired in PROMPT 1C.",
  ];
  let y = 715;
...
/**
 * Idempotently process a paid order: ensure modules_purchased rows exist,
 * upload a placeholder PDF, and create a `complete` reports row so the UI
 * can progress. Safe to re-run — uses upserts and existence checks.
 */
export async function runPlaceholderGeneration(order_id: string): Promise<void> {
  ...
  // If a complete report already exists for this intake, just merge any new modules.
  const { data: existingReport } = await sb
    .from("reports")
    .select("id, modules_array, download_token")
    .eq("intake_id", order.intake_id)
    .order("created_at", { ascending: false })
    .limit(1)
    .maybeSingle();

  // Owned modules across all orders for this intake
  const { data: ownedRows } = await sb
    .from("modules_purchased")
    .select("module_code")
    .eq("intake_id", order.intake_id);
  const owned = new Set<string>((ownedRows ?? []).map((r: any) => r.module_code));
  const modules: any[] = ["CORE", ...MODULE_CODES.filter((m) => owned.has(m))];

  // Intake metadata for the PDF
  ...
  const pdfBytes = await buildPlaceholderPdf({
    ...
    modules: modules.filter((m) => m !== "CORE") as ModuleCode[],
  });
...
// ============================================================
// Public: signed download URL for /download/$reportToken
// ============================================================
