...
So: **1 purchase → 1 reports row → 1 PDF / 1 token / 1 email.**

> ⚠️ **Correction (2026-06-06):** the production renderer `renderReportHtmlSafe()` renders
> **CORE only** — it does **not** render add-on modules. `renderAddon()` exists only inside the
> **deprecated** `renderReportHtml()` (marked "NOT USED BY PIPELINE. Do not reactivate"). So the
> delivered PDF today is **CORE-only even for bundles**; add-on content is generated by the AI but
> **never rendered/delivered**. Therefore BUNDLE-B's per-module PDFs for add-ons depend on a
> **production-safe add-on renderer**, which belongs to the module content track
> (MATERIAL-PACK-1 → MODULE-PROMPT-1 → add-on render → MODULE-DIAG-1) — that track is a hard
> prerequisite for delivering real add-on/bundle PDFs.
