...
| `.env.development` | **YES — tracked in git** | Stripe test publishable key (pk*test*...) — public key by design      | YELLOW FLAG — see below |
| `.env.production`  | **YES — tracked in git** | Stripe live publishable key (pk*live*...) — public key by design      | YELLOW FLAG — see below |
